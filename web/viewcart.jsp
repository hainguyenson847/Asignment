            ...
            <form action="pay" method="post" style="">
                <div class="row " style="margin-bottom: 500px">
                    ...
                    <c:forEach items="${sessionScope.cart.listItems}" var="li">
                        ...
                        <div class="col-lg-2 text-center border center-content">
                            ...
                            <button type="button" style="border: 1px grey solid; " onclick="
                                    ...
                                    if (a === ${li.product.quantity}) {
                                        ...
                                        var h = document.getElementById('tp${li.product.pid}');
                                        var ty = parseInt(document.getElementById('quan${li.product.pid}').value);
                                        var tt = parseInt(${Integer(li.product.salePrice * (1 - li.product.discount) )} / 1000) * parseInt(ty);
                                        var tb = document.getElementById('tp');
                                        var tm = parseInt(${(li.product.salePrice * (1 - li.product.discount))}) + parseInt(tb.value);
                                        var tr;
                                        var m = document.getElementById('tpp');
                                        var kk;
                                        if (tt >= 1000) {
                        ...
                        <div class="col-lg-2 text-center border center-content">
                            <c:if test="${li.product.discount > 0}">
                                <span id="tp${li.product.pid}" style="color: red; font-weight: bold">${Integer(li.product.salePrice * (1 - li.product.discount) / 1000) * sessionScope.cart.getQuantityByID(li.product.pid)}.000đ</span>
                            </c:if>
                            <c:if test="${li.product.discount == 0}">
                                <span id="tp${li.product.pid}" style="color: red; font-weight: bold">${Integer(li.product.salePrice / 1000) * sessionScope.cart.getQuantityByID(li.product.pid)}.000đ</span>
                            </c:if>
                        </div>  
