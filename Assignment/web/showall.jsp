<%-- 
    Document   : showall
    Created on : Jul 2, 2024, 11:32:54 PM
    Author     : DELL-PC
--%>
...
<html>
    ...
    <body style="margin: 0; padding: 0">
        ...
        <c:if test="${sessionScope.allcp == 'Tất cả sản phẩm'}">
            <div style="color: black" class="container">
                <form action="home" method="post" >
                    Sắp xếp theo
                    <select name="ord" onchange="this.form.submit();">
                        <option value="def" ${sessionScope.t == null? 'selected':''}>Mặc định</option>
                        <option value="asc" ${sessionScope.t == 'a'? 'selected':''}>Giá tăng dần</option>
                        <option value="desc" ${sessionScope.t == 'd'? 'selected':''}>Gía giảm dần</option>
                        <option value="az" ${sessionScope.t == 'az'? 'selected':''}>Tên A-Z</option>
                        <option value="za" ${sessionScope.t == 'za'? 'selected':''}>Tên Z-A</option>
                    </select>
                    <input name="category" value="home" type="text" style="display: none"/>
                ...
                <form action="home" method="post" >
                    ...
                    <select name="ord" onchange="this.form.submit();">
                        <option value="def" ${sessionScope.t == null? 'selected':''}>Mặc định</option>
                        <option value="10-50" ${sessionScope.t == '10'? 'selected':''}>10.000d - 50.000d</option>
                        <option value="50-100" ${sessionScope.t == '50'? 'selected':''}>50.000d - 100.000d</option>
                        <option value="100-500" ${sessionScope.t == '100'? 'selected':''}>100.000d - 500.000d</option>
                    </select>
                    <input name="category" value="home" type="text" style="display: none"/>
                    <noscript><input type=”submit” value=”Submit”></noscript>
                </form>
        ...
        <section>
            ...
            <div class="container my-5">
                ...
                <div class="row">
                    <c:forEach items="${sessionScope.listall}" var="li">
                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="card-footer d-flex align-items-end pt-3 px-0 pb-0 mt-auto">
                                        <button type="button" ${li.quantity == 0 ? 'disabled':''}   class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal${li.pid}" onclick="document.getElementById('quan${li.pid}').value = '1';" 
                                                style=" color: white; border: 1px pink solid;style${li.quantity == 0 ? 'background-color: #8b1b46':'background-color: #007aff'}  ">
                                        ...
                                        <a href="detail?pid=${li.pid}" class="btn btn-light border px-2 pt-1 icon-hover">Chi tiết</i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
