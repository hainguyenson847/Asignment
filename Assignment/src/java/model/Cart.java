...
public class Cart {
    ...
    public double getTotalPriceWithOutDiscount() {
        double total = 0;
        for (Item item : listItems) {
            total += Math.round(item.getQuantity() * item.getProduct().getSalePrice() * (1 - item.getProduct().getDiscount()) * 100) / 100;
        }
    ...
    public double getTotalMoney() {
        double total = 0;
        for (Item item : listItems) {
            total += Math.round(item.getQuantity() * (item.getSalePrice() * (1 - item.getProduct().getDiscount()))* 100) / 100.0;
        }
        total = Math.round(total * 100.0) / 100.0;
        return total;
    }
